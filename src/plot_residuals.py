...
for disp_var in DISP_VARS:
    ...
    for count_br,curr_vessel in enumerate(VESSELS):
        ...
        for count_date, curr_date in enumerate(unique_code_run_dates):
            ...
            mape_all = np.round(np.nanmean(np.abs((est_pred_all - data_all)/data_all*100)),0)
            ...
            mape_pred = np.round(np.nanmean(np.abs((pred_pred - data_pred)/data_pred*100)),0)
            ...
            mape_est = np.round(np.nanmean(np.abs((est_est - data_est)/data_est*100)),0)
