...
for i in df["Batch"].unique():
    ...
    # This code is specific to PVRIG project and can be discarded for templating purposes
...
scaled = scaler.transform(pre_scaled)
...
if ALL_INPUT_OPTIMIZATION:
    if BATCH in TEST_BATCHES:
        initial_feed = np.polyfit(t, df_test[df_test["Batch"] == BATCH]
            .sort_values("Day")
            .iloc[:, NUMBER_STATES],DEGREE)

        initial_gluc = np.array(
            df_test[df_test["Batch"] == BATCH]
            .sort_values("Day")
            .iloc[:, NUMBER_STATES+1]
        )

        initial_input = np.hstack([initial_feed,initial_gluc])
    else:
        initial_feed = np.polyfit(t, df_train[df_train["Batch"] == BATCH]
            .sort_values("Day")
            .iloc[:, NUMBER_STATES],DEGREE)
...
def optimization_output(input_matrix):
    ...
    # data = input_matrix.reshape(DEGREE+1, NUMBER_INPUTS)
    poly_feed = np.poly1d(input_matrix[:DEGREE])
    glucose = input_matrix[DEGREE+1:].reshape(DAYS,1)[:-1]
    pre_u = np.hstack([np.polyval(poly_feed,t_sim).reshape(DAYS,1)[:-1], glucose])
    u_sim = np.vstack([pre_u,np.array([0,0])])
...
def poly_opt_objective_func(input_matrix, info):
    """_summary_
...
def cell_count_constraint(input_matrix):  # Viability Constraint
    """_summary_
...
# y_out_opt, u_sim = optimization_output(initial_input)

# state_constraints = StateConstraints(y_out_opt, u_sim, scaler, DAYS, NUMBER_STATES, NUMBER_INPUTS)

# constraints = (
#     {"type": "ineq", "fun": state_constraints.minzero()},
#     # {"type": "ineq", "fun": state_constraints.cell_count},
#     {"type": "ineq", "fun": state_constraints.ammonium(MAX_AMMONIUM)},
#     {"type": "ineq", "fun": state_constraints.lactate(MAX_LACTATE)},
#     {"type": "ineq", "fun": state_constraints.feedpercent(average_vol, VOLUME_ERROR, MAX_FEED_PERCENT)},
#     {"type": "ineq", "fun": state_constraints.maxcsfr(average_vol, VOLUME_ERROR, MAX_CSFR)},
#     {"type": "ineq", "fun": state_constraints.mincsfr(average_vol, VOLUME_ERROR, MIN_CSFR)},
#     {"type": "ineq", "fun": state_constraints.glucose(GLUC_SETPOINT)},
# )

constraints = [
    {"type": "ineq", "fun": minzero_constraint},
    {"type": "ineq", "fun": cell_count_constraint},
...
if ALL_INPUT_OPTIMIZATION:
    result_feed = res.x[:DEGREE]
    result_glucose = res.x[DEGREE+1:].reshape(DAYS,1)
    pre_input = np.hstack([np.polyval(result_feed,t).reshape(DAYS,1)[:-1],result_glucose[:-1]])
    opt_input = np.vstack([pre_input,np.array([0,0])])
else:
